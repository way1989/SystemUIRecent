 ...
 *
 * HorizontalSwipeView.Java
 *
 * copy from android.widget.HorizontalScrollView for
 * scrolling one screen items
 *
...
public class HorizontalSwipeView extends FrameLayout {
    ...
    public HorizontalSwipeView(Context context, AttributeSet attrs, int defStyle) {
...
//        setFillViewport(a.getBoolean(android.R.styleable.HorizontalScrollView_fillViewport, false));
    ...
    public void addView(View child) {
        if (getChildCount() > 0) {
            throw new IllegalStateException("HorizontalSwipeView can host only one direct child");
        }
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    boolean isOverScroll = getScrollX() >= range - getPerScreenWidth();
                    if(deltaX > 0 && isOverScroll){
                        //specialize for last page drag
                        return true;
                    }
                    mOverscrollDistance = 200;
            ...
            case MotionEvent.ACTION_UP:
                if (mIsBeingDragged) {
                    ...
                    float xVel = (int) velocityTracker.getXVelocity(mActivePointerId);
                    boolean negative = xVel < 0;

                    float yVel = (int) velocityTracker.getYVelocity(mActivePointerId);

                    float vel = (float) Math.hypot(xVel, yVel);
                    if (negative) {
                        vel = -vel;
                    }

                    if((vel > SNAP_VELOCITY) && mCurPage > 0) {
                        //we will go to previous screen
                        snapToScreen(mCurPage - 1);
                    } else if((vel < -SNAP_VELOCITY) && (mCurPage < getPageCount() - 1)) {
                        //we will go to next screen
                        snapToScreen(mCurPage + 1);
                    } else {
                        //we will calculate the postion of scroolX to decide scrolling which screen.
                        snapToDestination();
                    }
                    mActivePointerId = INVALID_POINTER;
                    mIsBeingDragged = false;
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_SCROLL: {
                    if (!mIsBeingDragged) {
                        ...
                        if (hscroll != 0) {
                            final int delta = (int) (hscroll * getHorizontalScrollbarHeight());
                            final int range = getScrollRange();
...
//            invalidateParentIfNeeded();
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        ...
        info.setClassName(HorizontalSwipeView.class.getName());
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(HorizontalSwipeView.class.getName());
    ...
    private void snapToDestination() {
        ...
        final int perScreenWidth = getPerScreenWidth();
        int destPage = (scrollX + perScreenWidth / 2 ) / perScreenWidth ;

        snapToScreen(destPage);
    }
...
//        }
        ...
        if(count > 0){
            scrollRange = getPerScreenWidth() * getPageCount();
        }
        return scrollRange;
    }
    ...
    void snapToScreen(int whichPage) {
        // TODO Auto-generated method stub
        mCurPage = whichPage;
        ...
        int dx = 0;
        int screenWidth = getPerScreenWidth();
        dx = mCurPage * screenWidth - getScrollX();
        mScroller.startScroll(start, 0, dx, 0, 200/*Math.abs(dx) * 2*/);
        invalidate();
    }
}
